...
class TreeOfThoughts:
    ...
    def naive_solve(self, args, task, idx, to_print=True):
        ...
        ys = self.thought_generator.get_samples(task, x, '', args.n_generate_sample, args.prompt_sample, stop=None)
        return ys, {}
    ...
    def solve(self, args, task, idx, to_print=True):
        ...
        for step in range(task.steps):
            ...
            if args.method_generate == 'sample':
                new_ys = [
                    self.thought_generator.get_samples(
                        task, x, y, args.n_generate_sample, prompt_sample=args.prompt_sample, stop=task.stops[step])
                    for y in ys]
            elif args.method_generate == 'propose':
                new_ys = [self.thought_generator.get_proposals(task, x, y) for y in ys]
            new_ys = list(itertools.chain(*new_ys))
            ids = list(range(len(new_ys)))
            # evaluation
            if args.method_evaluate == 'vote':
                values = self.state_evaluator.get_votes(task, x, new_ys, args.n_evaluate_sample)
            elif args.method_evaluate == 'value':
                values = self.state_evaluator.get_values(task, x, new_ys, args.n_evaluate_sample)
...
class TextTask:
    ...
    def test_output(self, idx: int, output: str):
        ...
        prompt = DefaultPrompt.score_prompt + output
    ...
    def standard_prompt_wrap(x: str, y: str = '') -> str:
        return DefaultPrompt.standard_prompt.format(input=x) + y

    @staticmethod
    def cot_prompt_wrap(x: str, y: str = '') -> str:
        return DefaultPrompt.cot_prompt.format(input=x) + y

    @staticmethod
    def vote_prompt_wrap(x: str, ys: list) -> str:
        prompt = DefaultPrompt.vote_prompt
    ...
    def compare_prompt_wrap(x: str, ys: list) -> str:
        ...
        ys = [y.split('Passage:\n')[-1] for y in ys]
        prompt = DefaultPrompt.compare_prompt + f'Passage 1:\n{ys[0]}\n\nPassage 2:\n{ys[1]}\n'
        return prompt
...
task = Game24Task()
ys, infos = TreeOfThoughts.solve(args, task, 900)
print(ys[0])