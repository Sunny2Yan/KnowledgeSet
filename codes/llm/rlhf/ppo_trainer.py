...
class PPOConfig:
    # 1. common parameters
    exp_name: str = os.path.basename(sys.argv[0])[: -len(".py")]  # 本次实验名称
    seed: int = 0
    log_with: Optional[Literal["wandb", "tensorboard"]] = None
    task_name: Optional[str] = None  # used only for tracking purposes
    ...
    # tracker的参数 (e.g. python ppo.py --tracker_kwargs='{"wandb": {"entity": "my_wandb_entity", "name": "my_exp_name"}}'
    tracker_kwargs: JSONDict = field(default_factory=dict)
    accelerator_kwargs: JSONDict = field(default_factory=dict)
    # 加速器项目配置的关键字参数 (e.g. `logging_dir`)
    project_kwargs: JSONDict = field(default_factory=dict)
    tracker_project_name: str = "trl"  # Name of project to use for tracking
    ...
    init_kl_coef: Optional[float] = 0.2  # 初始KL惩罚系数(used for adaptive and linear control)
    kl_penalty: Literal["kl", "abs", "mse", "full"] = "kl"  # Kl惩罚选项
    target: Optional[float] = 6  # 自适应KL控制的目标KL值
...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if isinstance(ref_model, SUPPORTED_ARCHITECTURES):
            ...
            # num_shared_layers默认为全部，即，reference_model = model
    ...
    def step(
        ...
        if self.config.score_clip is not None:
            # Score clipping  torch.clip(input, _min, _max) = min(max(x, _min), _max)
    ...
    def prepare_model_inputs(self, queries: torch.Tensor, responses: torch.Tensor):
        if self.is_encoder_decoder:
            ...
            # 将question_tensor与response_tensor拼接
    ...
    def batched_forward_pass(
        ...
        model_inputs: dict,  # {input_ids: ”question_tensor+response_tensor“, input_mask}
    ...
    def _kl_penalty(self, logprob: torch.FloatTensor, ref_logprob: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.config.kl_penalty == "full":  # kl散度
