...
class ParallelTransformerBlock(nn.Module):
    def __init__(
        ...
        self.fused_dims = (
        attn_inner_dim, dim_head, dim_head, (ff_inner_dim * 2))
...
class PaLM(nn.Module):
    ...
    def add_finetune_params(self, scope, lora_r=None):
        ...
        dim, dim_head, heads, r, device = self.dim, self.dim_head, self.heads, default(
            lora_r, self.lora_r), self.device
    ...
    def merge_finetune_params(self, scope):
        ...
        for layer, (lora_q, lora_k, lora_v, lora_o) in zip(self.layers,
                                                           lora_modules):
            block = layer.fn
            ...
            lora_qkv_weight, _ = pack(
                [lora_q.weight, lora_k.weight, lora_v.weight], 'i *')
            lora_qkv_weight = F.pad(lora_qkv_weight, (
            0, fused_proj_out_dim - lora_qkv_weight.shape[1]))
    ...
    def generate(
        ...
        for _ in wrapper_fn(range(sample_num_times)):
            logits, embeds = self.forward(out,
                                          return_logits_with_embedding=True,
                                          **kwargs)
    ...
    def forward(
        ...
        for layer, finetune_modules in zip_longest(self.layers,
                                                   finetune_modules):
